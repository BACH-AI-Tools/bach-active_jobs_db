...
4. 🔍 搜尋或找到本伺服器（`bach-active_jobs_db`）
5. 🎉 點擊 **「安裝 MCP」** 按鈕
6. ✅ 完成！即可在您的應用中使用
...
這是一個使用 [FastMCP](https://fastmcp.wiki) 自動生成的 MCP 伺服器，用於存取 Active Jobs Db API。
...
**注意**: 請將 `E:\path\to\active_jobs_db\server.py` 替換為實際的伺服器檔案路徑。
